...
auto_update_script() {
  ...
  wget --no-check-certificate https://neurorobotics-files.net/index.php/s/83zqkdp5PXQXMzz -O /tmp/$scriptname 2>/dev/null
...
restore() {
  container=$1
  var="${container}_image"
  declare "${container}_image=$($DOCKER_CMD ps -a | grep -w $container | awk '{print $2}')"
  declare "nrpversion=$( echo "${!var}" | cut -d ":" -f2)"
  set +e
  stop $1
...
setup_nrp(){
  ...
  if [ $nrpversion == "legacy" ]; then
      ...
      $DOCKER_CMD exec nrp bash -c "/bin/sed -e 's/localhost:9000/"$frontend_ip":9000/' -i /home/bbpnrsoa/nrp/src/VirtualCoach/hbp_nrp_virtual_coach/hbp_nrp_virtual_coach/config.json"
  elif [ $nrpversion == "latest" ]; then
      $DOCKER_CMD exec nrp bash -c '{ cd /home/bbpnrsoa/nrp/src/Models && git config remote.origin.fetch "+refs/heads/master*:refs/remotes/origin/master" && sudo git checkout master && sudo git pull; } || { cd /home/bbpnrsoa/nrp/src && sudo find Models/ -not -name "Models" -delete && sudo git clone --progress --branch=development https://bitbucket.org/hbpneurorobotics/Models.git Models/; }'
      echo -e "${BLUE}Cloning template experiments, this may take a while${NC}"
      $DOCKER_CMD exec nrp bash -c '{ cd /home/bbpnrsoa/nrp/src/Experiments && git config remote.origin.fetch "+refs/heads/development:refs/remotes/origin/development" && sudo git checkout development && sudo git pull; } || { cd /home/bbpnrsoa/nrp/src && sudo find Experiments/ -not -name "Experiments" -delete && sudo git clone --progress --branch=development https://bitbucket.org/hbpneurorobotics/Experiments.git Experiments/; }'
      $DOCKER_CMD exec nrp bash -c "/bin/sed -e 's/localhost:9000/"$frontend_ip":9000/' -i /home/bbpnrsoa/nrp/src/VirtualCoach/hbp_nrp_virtual_coach/pynrp/virtual_coach.py"
  else
  ...
  $DOCKER_CMD exec nrp bash -c "/bin/sed -e 's/localhost:9000/"$frontend_ip":9000/' -i /home/bbpnrsoa/nrp/src/ExDBackend/hbp_nrp_commons/hbp_nrp_commons/workspace/Settings.py"
...
setup_frontend() {
  ...
  $DOCKER_CMD exec frontend bash -c "/bin/sed -e '/\"localhost\": {/,/\"gzweb\": {/c\"localhost\": {\n\t\t\"internalIp\": \"http:\/\/172.19.0.3:8080\",\n\t\"gzweb\": {' -i /home/bbpnrsoa/nrp/src/nrpBackendProxy/config.json"
...
if [ "$2" == "legacy" ] ; then
    nrpversion="legacy"
    nrp_image="hbpneurorobotics/nrp:legacy"
    frontend_image="hbpneurorobotics/nrp_frontend:legacy"
elif [ "$2" == "latest" ] ||  [ -z "$2" ] ; then
    nrpversion="latest"
    nrp_image="hbpneurorobotics/nrp:latest"
    frontend_image="hbpneurorobotics/nrp_frontend:latest"
else
    echo -e "${RED}[ERROR] Wrong version specified ${NC}"
    echo -e "$usage"
fi
...
if $is_win
then
	external_frontend_ip="localhost"
	external_nrp_ip="localhost"
else
